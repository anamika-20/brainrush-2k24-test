...
const page = () => {
  ...
  const [selectedTopics, setSelectedTopics] = useState({ topic1: '', topic2: '', topic3: '' });
    ...
    try {
      ...
      if (userResponse) {
        const { data } = await axios.patch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/api/quiz`, { quizIds: Object.values(selectedTopics) }
        );
        if (data.success) {
  ...
  const filterOptions = (exclude) => {
    return options.filter(option => !Object.values(selectedTopics).includes(option._id) || exclude.includes(option._id));
  };
      ...
      >
        ...
        <div>
          <div className="mb-6 h-px w-full bg-bgGray ">
            <div className="h-px bg-background " style={{ width: "100%" }}></div>
          </div>
